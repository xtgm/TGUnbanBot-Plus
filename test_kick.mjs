...
// ---------- [1b2] /sa USER_ID_INVALID 确定性错误不重试:精准提示 ----------
console.log('\n[1b2] /sa USER_ID_INVALID 不重试且精准提示');
{
	resetCalls();
	const fakeCtx = { waitUntil: (p) => { Promise.resolve(p).catch(() => {}); } };
	sandbox.fetch = makeFetchMock({
	...
	const banCalls = callsOf('banChatMember');
	assert('/sa USER_ID_INVALID 确定性错误不重试', banCalls.length === 2 && banCalls.filter((c) => String(c.body.chat_id) === '-1002').length === 1, `实际 ${banCalls.length}`);
	const dmSend = callsOf('sendMessage').find((c) => String(c.body.chat_id) === '999');
...
// ---------- [10b2] 群内 /be USER_ID_INVALID 确定性错误不重试 ----------
console.log('\n[10b2] 群内 /be USER_ID_INVALID 不重试');
{
	resetCalls();
	const fakeCtx = { waitUntil: (p) => { Promise.resolve(p).catch(() => {}); } };
	...
	const banCalls = callsOf('banChatMember');
	assert('/be USER_ID_INVALID 确定性错误不重试', banCalls.length === 2 && banCalls.filter((c) => String(c.body.chat_id) === '-1002').length === 1, `实际 ${banCalls.length}`);
	const dmSend = callsOf('sendMessage').find((c) => String(c.body.chat_id) === '999');
	assert('/be USER_ID_INVALID:私聊回执存在', !!dmSend);
	assert('/be USER_ID_INVALID:显示封禁成功 1/2', dmSend.body.text.includes('Telegram 群封禁/预封成功 1/2'));
	assert('/be USER_ID_INVALID:提示 Telegram 当前无法识别', dmSend.body.text.includes('Telegram 当前无法识别该 TGID'));
}
...
console.log('\n[10b3] 群内 /be 首次加黑成功但 Telegram 无法识别 TGID');
{
	...
	assert('/be USER_ID_INVALID 每个群只请求一次', callsOf('banChatMember').length === 2);
...
console.log('\n[11] 群内 /be 批量');
{
	...
	assert('批量 /be 显示可点击姓名', dmSend.body.text.includes('<a href="tg://user?id=100">用户甲</a>') && dmSend.body.text.includes('<a href="tg://user?id=200">用户乙</a>'));
	assert('批量 /be 同时显示 username', dmSend.body.text.includes('<code>@user100</code>'));
	assert('批量 /be 仅 username 时可点击', dmSend.body.text.includes('<a href="tg://user?id=300">@user300</a>'));
	assertExactProfileQueries('批量 /be 每个目标只查询来源群一次资料', ['100', '200', '300'], '-1001');
...
console.log('\n[11a] 群内 /sa 小批量用户资料');
{
	...
	const update = {
		message: {
			...
			chat: { id: -1001, type: 'supergroup' },
			from: { id: 999, is_bot: false, first_name: '主人' },
			text: '/sa 410,420 批量广告',
		},
	};
	await handler.fetch(new Request('https://x.com/', { method: 'POST', body: JSON.stringify(update) }), env, fakeCtx);
	const blacklist = JSON.parse(env.DB._store.get('blacklist') || '[]');
	assert('小批量 /sa 两个用户全部加黑', blacklist.length === 2 && blacklist.every((entry) => entry.reason === 'sa'));
	assert('小批量 /sa 执行 2 用户 × 2 群封禁', callsOf('banChatMember').length === 4);
	assertExactProfileQueries('小批量 /sa 每个目标只查询来源群一次资料', ['410', '420'], '-1001');
	const dmSend = callsOf('sendMessage').find((c) => String(c.body.chat_id) === '999');
	assert('小批量 /sa 可解析用户显示可点击姓名', dmSend.body.text.includes('<a href="tg://user?id=410">广告甲</a>') && dmSend.body.text.includes('<code>@spam410</code>'));
	assert('小批量 /sa 无法解析时回退可点击 TGID', dmSend.body.text.includes('<a href="tg://user?id=420">420</a> <code>420</code>'));
}
...
console.log('\n[11b] 群内 /be 20 个 TGID → D1 批量任务');
{
	...
	assert('批量 /be 的 40 个群操作按 24 上限分 2 次 Queue', job.autoRunCount === 2);
	...
	assert('批量 /be Queue 每 20 人只执行 1 条 D1 mutation', env.DB._mutationCalls.length === 1 && env.DB._mutationCalls[0].bound.length === 100);
	assert('批量 /be 自动踢人全部完成', callsOf('banChatMember').length === 40);
	assert('大批量 /be Queue 路径不查询用户资料', callsOf('getChatMember').length === 0);
...
console.log('\n[11c] 群内 /sa 20 个 TGID → D1 批量任务');
{
	...
	assert('批量 /sa 的 40 个群操作按 24 上限分 2 次 Queue', job.autoRunCount === 2);
	...
	assert('大批量 /sa Queue 路径不查询用户资料', callsOf('getChatMember').length === 0);
}
...
console.log('\n[11g] 4×7 进入 Queue');
{
	...
	await handler.fetch(new Request('https://x.com/', {
		method: 'POST',
		body: JSON.stringify({ message: { message_id: 850, chat: { id: 999, type: 'private' }, from: { id: 999, is_bot: false }, text: `/be ${ids.join(',')}` } })
	}), env, fakeCtx);
	await drainPending(pending);
	const job = JSON.parse([...env.DB._jobs.values()][0].payload);
...
console.log('\n[11g2] 单用户逐群探测预算');
{
	...
	await handler.fetch(new Request('https://x.com/', {
		method: 'POST',
		body: JSON.stringify({ message: { message_id: 8502, chat: { id: 999, type: 'private' }, from: { id: 999, is_bot: false }, text: '/be 51999' } })
	}), env, fakeCtx);
	await drainPending(pending);
	const job = JSON.parse([...env.DB._jobs.values()][0].payload);
	assert('1×23 /be 因逐群探测估算 103 进入 Queue', job.totals.operations === 23 && job.totals.estimatedSubrequests === 103);
...
console.log('\n[11g3] 普通管理员当前群一次鉴权预算');
{
	...
	await handler.fetch(new Request('https://x.com/', {
		method: 'POST',
		body: JSON.stringify({ message: { message_id: 8503, chat: { id: Number(currentGroup), type: 'supergroup' }, from: { id: 777, is_bot: false }, text: '/be 51998' } })
	}), env, fakeCtx);
	await drainPending(pending);
	...
	assert('普通管理员同步 /be 仍写入 D1 全局黑名单', env.DB._rows.has('51998'));
}
...
console.log('\n[11h] 50/51 人边界');
{
	...
	await handler.fetch(new Request('https://x.com/', {
		method: 'POST',
		body: JSON.stringify({ message: { message_id: 851, chat: { id: 999, type: 'private' }, from: { id: 999, is_bot: false }, text: `/be ${fifty.join(',')}` } })
	}), env, fakeCtx);
	await drainPending(pending);
	...
	for (const command of ['/be', '/sa', '/unban']) {
		resetCalls();
...
console.log('\n[11j] Queue 重复投递原子租约');
{
	...
	await handler.fetch(new Request('https://x.com/', {
		method: 'POST',
		body: JSON.stringify({ message: { message_id: 856, chat: { id: 999, type: 'private' }, from: { id: 999, is_bot: false }, text: `/be ${ids.join(',')}` } })
	}), env, fakeCtx);
...
console.log('\n[11j1] Queue 租约 fencing');
{
	...
	await handler.fetch(new Request('https://x.com/', {
		method: 'POST',
		body: JSON.stringify({ message: { message_id: 8561, chat: { id: 999, type: 'private' }, from: { id: 999, is_bot: false }, text: `/be ${ids.join(',')}` } })
	}), env, fakeCtx);
	const queuedJob = JSON.parse([...env.DB._jobs.values()][0].payload);
...
console.log('\n[11j1a] Queue 当前 GROUP_IDS 过滤');
{
	...
	await handler.fetch(new Request('https://x.com/', {
		method: 'POST',
		body: JSON.stringify({ message: { message_id: 8562, chat: { id: 999, type: 'private' }, from: { id: 999, is_bot: false }, text: `/be ${ids.join(',')}` } })
	}), env, fakeCtx);
	const queuedJob = JSON.parse([...env.DB._jobs.values()][0].payload);
...
console.log('\n[11j1b] Queue send 失败 fencing');
{
	...
	await handler.fetch(new Request('https://x.com/', {
		method: 'POST',
		body: JSON.stringify({ message: { message_id: 8563, chat: { id: 999, type: 'private' }, from: { id: 999, is_bot: false }, text: `/be ${ids.join(',')}` } })
	}), env, fakeCtx);
	const queuedJob = JSON.parse([...env.DB._jobs.values()][0].payload);
...
console.log('\n[11j2] Queue D1 失败恢复');
{
	...
	await handler.fetch(new Request('https://x.com/', {
		method: 'POST',
		body: JSON.stringify({ message: { message_id: 859, chat: { id: 999, type: 'private' }, from: { id: 999, is_bot: false }, text: `/be ${ids.join(',')}` } })
	}), env, fakeCtx);
	await drainPending(pending);
...
console.log('\n[11k] Telegram 瞬态错误重试');
{
	for (const scenario of ['429', 'network', '5xx']) {
		...
		await handler.fetch(new Request('https://x.com/', {
			method: 'POST',
			body: JSON.stringify({ message: { message_id: 857, chat: { id: 999, type: 'private' }, from: { id: 999, is_bot: false }, text: `/be ${scenario === '429' ? '56001' : '56002'}` } })
		}), env);
		assert(`/be ${scenario} 仅延迟重试一次并成功`, callsOf('banChatMember').length === 2);
	}
}
{
	resetCalls();
...
console.log('\n[16] 普通管理员当前群鉴权 + 全局封禁');
{
	...
	await handler.fetch(new Request(`https://x.com/`, {
		method: 'POST',
		body: JSON.stringify({ message: { message_id: 1, chat: { id: -1001, type: 'supergroup' }, from: { id: 6666, is_bot: false }, text: '/be 555' } }),
	}), env, fakeCtx);
	let blacklist = JSON.parse(env.DB._store.get('blacklist') || '[]');
	assert('A 群不是管理员 → /be 不写 D1', blacklist.length === 0);
	...
	await handler.fetch(new Request(`https://x.com/`, {
		method: 'POST',
		body: JSON.stringify({ message: { message_id: 2, chat: { id: -1002, type: 'supergroup' }, from: { id: 6666, is_bot: false, first_name: 'B群管理员' }, text: '/be 555' } }),
	}), env, fakeCtx);
	blacklist = JSON.parse(env.DB._store.get('blacklist') || '[]');
	assert('B 群是管理员 → /be 写入 D1 全局黑名单', blacklist.length === 1 && blacklist[0].id === '555');
...
console.log('\n[18] 当前群查询失败即拒绝，不跨群借权');
{
	...
	await handler.fetch(new Request(`https://x.com/`, { method: 'POST', body: JSON.stringify(update) }), env, fakeCtx);
	const blacklist = JSON.parse(env.DB._store.get('blacklist') || '[]');
	assert('当前群查询失败 → /be 不写 D1', blacklist.length === 0);
...
// ---------- [19a] 普通 Telegram 管理员不能私聊执行 /be /sa ----------
console.log('\n[19a] 普通管理员私聊 /be /sa 拒绝');
{
	resetCalls();
	const fakeCtx = { waitUntil: (p) => { Promise.resolve(p).catch(() => {}); } };
	sandbox.fetch = makeFetchMock({
	...
	const env = { ...baseEnv, DB: makeFakeDB([]) };
	for (const [messageId, text] of [[1, '/be 601'], [2, '/sa 602 广告']]) {
		await handler.fetch(new Request('https://x.com/', {
			method: 'POST',
			body: JSON.stringify({ message: { message_id: messageId, chat: { id: 6666, type: 'private' }, from: { id: 6666, is_bot: false }, text } }),
		}), env, fakeCtx);
	}
	assert('普通管理员私聊 /be /sa 不写 D1', !env.DB._rows.has('601') && !env.DB._rows.has('602'));
	assert('普通管理员私聊 /be /sa 不执行全群封禁', callsOf('banChatMember').length === 0);
	...
	assert('普通管理员私聊 /be /sa 均收到权限不足', replies.length === 2 && replies.every((c) => c.body.text.includes('权限不足')));
}

// ---------- [19b] 普通管理员除 /be /sa 外无其它管理命令权限 ----------
console.log('\n[19b] 普通管理员其它管理命令拒绝');
{
	resetCalls();
	const fakeCtx = { waitUntil: (p) => { Promise.resolve(p).catch(() => {}); } };
