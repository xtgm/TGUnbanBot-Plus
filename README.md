...
- **名片广告检测(本版重点)**:广告号常用**分享联系人名片**的形式发广告(名片显示名写"假钞交流群"、电话填国际号),名片本身无 `text`/`caption`,旧版完全漏检(只删消息不加黑不踢)。本版把名片内容(显示名/电话号/vcard)纳入检测:① 名片含**国际电话号(+国家码,排除 +86 中国号)**→ 强特征直杀,删+黑+全群踢;② 名片显示名命中词库(如"假钞")→ 词库分 + 名片分(+1)叠加达阈值判定;③ **正常分享本地联系人名片不误杀**(中国号 + 无敏感词,只 +1 分不达阈值)。名片广告也会被缓存供 `/recent`/`/learnlast` 学习。
...
**本版已修**:检测会把名片内容(显示名/电话号/vcard)一并识别:

1. **名片含国际电话号**(带 `+国家码`,排除 +86 中国号)→ **强特征直杀**,删消息 + 加黑 + 全群踢。截图里 `+1 870 337 4069` 就属于这种。
2. **名片显示名命中词库**(如"假钞"在 fraud 分类)→ 词库分 + 名片分(+1)叠加,达阈值判定。
