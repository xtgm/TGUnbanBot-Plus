...
- **发言人身份检测(本版重点,抓"名字/简介本身是广告"的号)**:有些广告号消息正文是正常内容(甚至蹭"chatgpt plus"这类正经词),但**名字或简介本身就是广告**(如 `сЛuВы Со ВпucoК`、名字/简介里塞 `t.me/频道`、`卡网:pay.ldxp.cn`、网址)。本版**只检测发言人的身份字段(名字/用户名/简介),不碰消息正文**——所以群里正常聊 chatgpt、分享 t.me 链接**绝不会被误杀**,只有"身份本身是广告"的人才被处置:① **名字/用户名**含网址 / `t.me` 链接 / `@提及` / 域名 / 身份广告词(卡网/发卡/车队/号商等)→ 直接秒杀,零 API 成本每条都查;② **简介 bio**(`AD_CHECK_BIO` **默认开启**)含同类特征 → 调 `getChat` 拉 bio 检测,**带 KV 缓存**(同一用户默认 6 小时内不重复查,大幅降低 API 成本)。身份广告词可用 `/addword identity <词>` 热更新,或 `/importdefault` 一键导入推荐 identity 词库。
  - **参考杀神(GodKnowYouBot)实现 bio 检测**:杀神官网列出它筛查 first/last name、bio、头像、消息文本、图片、贴纸。本项目已对齐其中的**名字 + bio + 文本**检测(头像需 OCR,Worker 做不了)。诚实说明:bio 检测**不存在"完美"**——Telegram Bot API 的 `getChat` 只能拿到公开 username 或与 bot 交互过用户的 bio,**纯陌生群成员的 bio 杀神也拿不到**(它官网自承 1-3% 漏判率),这是平台限制,非代码可绕过。本项目在 Worker 环境下已做到最优:名字检测零成本每条查 + bio 检测带缓存。
  - **重要区分**:`chatgpt/plus/codex` 等是正经 AI 名字,**绝不会**被加进广告词;身份检测抓的是"名字里放网址/卡网"这种正常用户绝不会有的特征,不是匹配 AI 词。
