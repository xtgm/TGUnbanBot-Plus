...
- **D1 数据库（唯一存储后端）**：完全基于 Cloudflare D1 数据库，免费层每天 10 万次写入不会溢出。黑名单、广告词库、学习样本、消息缓存全部存 D1。保留一次性迁移入口 `/{TOKEN}/migrate` 供从原项目（KV）升级的用户把存量数据导入 D1。
...
- **广告词库存 D1 + 主人热更新**:为隐私起见,代码内置词库**默认全部清空**(GitHub 公开仓库零敏感词),词库存在 D1 里。主人用 Telegram 命令热更新,**改词不用重新部署**:`/importdefault` 一键导入推荐词库、`/addword` 加词、`/delword` 删词、`/listwords` 查看。仅主人(OWNER_ID)可改(词库是全局配置,误加影响所有群)。
...
| 命令 | 使用位置 | 说明 |
| --- | --- | --- |
| `/ban 用户ID` | 私聊 / 群内 | 加入 D1 黑名单（reason=manual）+ **遍历所有 GROUP_IDS 真踢出群**。**支持批量**：`/ban 123,456,789`（半角 / 全角逗号 / 空格 / 换行均可，单次最多 50 个）。**群内执行时机器人发短闪屏（5 秒自动撤回）+ 私聊管理员发完整详情**，避免长结果污染群消息流。 |
| `/unban 用户ID` | 私聊 / 群内 | 从 D1 黑名单移除。**支持批量**：`/unban 123,456,789`（同上分隔符与上限）。群内执行时同样发闪屏 + 私聊详情。 |
| `/spam` | 群内回复 | 把被回复用户加入 D1 黑名单（reason=spam）+ **遍历所有 GROUP_IDS 真踢出群** + **删除被回复的垃圾消息** |
...
仅 `OWNER_ID` 配置的主人可用。**对普通用户、群管理员、超级管理员完全无反应**(群内静默、私聊提示权限不足),且这些命令不注册到 BotFather 命令菜单,天然隐藏。主人**私聊机器人**发 `/help` 可随时展开下表。词库/样本存在 D1,改动无需重新部署,立即生效:
...
├── test_kick.mjs    # 真踢人闭环 + 广告学习离线测试（227 项）
├── README.md
└── LICENSE
```
...
| 变量名 | 说明 |
| --- | --- |
| `TOKEN` | 初始化入口密钥。访问 `https://你的Worker域名/{TOKEN}` 注册 Webhook + 注册命令；访问 `https://你的Worker域名/{TOKEN}/migrate` 触发旧 KV 数据迁移到 D1（仅升级用户需要）；访问 `https://你的Worker域名/{TOKEN}/export` 导出黑名单；访问 `https://你的Worker域名/{TOKEN}/purge` 一次性清扫存量黑名单仍在群里的人。建议用随机长字符串。 |
...
| 绑定 | binding 名 | 用途 |
| --- | --- | --- |
| D1 Database | `DB` | **唯一存储后端**。黑名单、广告词库、学习样本、消息缓存全部存 D1。表结构由代码自动建立，首次写入自动建表。 |
...
### 7. 存量 KV 数据迁移到 D1（仅从原项目升级时需要）

如果你之前部署过原项目（使用 KV 存储）且现在升级到本项目（D1 存储），可以一键把存量 KV 黑名单、广告词库、学习样本导入 D1：

**前提**：需要在 `wrangler.toml` 中**同时绑定旧的 KV 和新的 D1**，迁移完成后可以删除 KV 绑定。
...
迁移幂等，重复触发安全。完成后可从 `wrangler.toml` 中移除 KV 绑定。
...
### 一次性迁移入口（受 TOKEN 保护，仅升级用户需要）

```http
GET /{TOKEN}/migrate
```

把旧项目的 KV 黑名单、广告词库、学习样本全量幂等导入 D1。要求同时绑定旧 KV 和 D1（迁移完成后可删除 KV 绑定）。
...
扫描当前 D1 黑名单 × 所有 `GROUP_IDS`，对每个**仍在群里**的人调 `banChatMember` 真踢出群。Telegram 状态为 `kicked` / `left` 的会跳过。返回 JSON 详情：
...
**原因**：D1 表结构由代码自动创建，但**存量 KV 数据不会自动迁移**。
**解决**：浏览器访问 `https://你的Worker域名/{TOKEN}/migrate` 触发一次性迁移（需同时绑定旧 KV 和 D1），幂等可重复触发。迁移完成后可删除 KV 绑定。
...
5. 注意:这些命令**仅主人(OWNER_ID)可用**,且需在**私聊**机器人时发送(群里发会被静默忽略)。词库存 D1,改完即时生效,无需 `wrangler deploy`。
