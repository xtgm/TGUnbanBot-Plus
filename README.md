...
- **广告自动检测**（可选 `AD_FILTER_ENABLED` 启用）：普通成员发的疑似广告 → **自动删消息 + 加黑 + 全群踢出 + 通知主人**,零人工。采用**多维度加权评分 + 强特征直杀**:t.me/+ 私有群邀请链接、国际电话号这类强特征单独命中即处置;金融/色情/引流/诈骗分类词库加权评分,达阈值才判定(降低误杀)。检测覆盖消息文本/图片说明/用户名字/内嵌链接。**不检测 emoji**(避免误杀正常用户发的表情包)。管理员豁免,默认关闭。
...
- **`/spam` 上报学习 → 精准查杀**(参考 GKY 神膠bot 的穿透上报系统):**主人**回复一条漏检广告发 `/spam`,系统在加黑+踢+删之外,还会**把广告内容学习入库**:① 整句指纹(归一化后,以后相同广告精确秒杀,抗加空格/标点/全半角变体)② 自动提取特征词组进词库(抓"改几个字"的变体)。以后任何人再发相同/相似广告,自动删+黑+踢。普通管理员 `/spam` 只加黑踢不学习。主人用 `/listsamples` `/delsample` `/clearsamples` 管理样本库(无人为上限)。
- **`/learn` `/learnlast` 应对"已删消息"**:GKY 等机器人会删广告,导致无法 `/spam` 回复。两个解法:① `/learn <粘贴广告文本>` 主人直接粘贴文字学习,不依赖回复 ② bot 自动缓存最近"疑似广告"群消息(含链接/@提及/长数字/长文本),GKY 删完主人立刻 `/learnlast` 学到那条 + 加黑全群踢发送者。缓存只存疑似广告省 KV 写入,用 `ctx.waitUntil` 异步不阻塞。
...
### 主人专属(`OWNER_ID`)— 广告词库热更新

仅 `OWNER_ID` 配置的主人可用,在**私聊机器人**发送(群内静默)。词库存在 KV,改词无需重新部署,立即生效:

| 命令 | 说明 |
| --- | --- |
| `/importdefault` | 一键导入推荐词库(金融/色情/引流/诈骗四类)到 KV,与现有词去重合并 |
| `/addword [分类] 词1 词2 ...` | 加词。分类可选:`finance`/`porn`/`spam`/`fraud`/`general`/`whitelist`,默认 `general`。例:`/addword fraud 杀猪盘 刷信誉` |
| `/delword 词1 词2 ...` | 从所有分类删除指定词 |
| `/listwords` | 查看当前 KV 词库全部内容(分类 + 数量) |
| `/listsamples` | 查看 `/spam` 学习的广告样本指纹(最近 50 条 + 总数) |
| `/delsample 序号或关键词` | 删除某条学习样本(序号见 /listsamples,或按关键词匹配删) |
| `/clearsamples confirm` | 清空全部学习样本(需带 `confirm` 二次确认) |
| `/learn 广告文本` | **直接粘贴**广告文字学习(不需要回复消息)。解决 GKY 已删消息无法 `/spam` 的痛点 |
| `/learnlast [N]` | 学习最近 N 条(默认 1)被缓存的疑似广告 + 加黑全群踢发送者。GKY 删消息后立刻发即可 |

> 词库是**全局配置**,影响所有群的广告检测,所以仅限主人改。代码内置词库默认为空(GitHub 零敏感词),首次启用广告检测后建议先 `/importdefault` 导入推荐词库。
>
> **`/spam` 学习**:主人回复一条漏检广告发 `/spam`,除加黑+踢+删外,还会把广告**整句指纹**入库(以后相同广告精确秒杀)并**自动提取特征词**进词库。普通管理员 `/spam` 只加黑踢不学习。
>
> **已删消息怎么学**:GKY 神膠bot 等机器人会删广告导致无法 `/spam` 回复。两个办法:① `/learn 粘贴广告文字`(不依赖回复)② bot 自动缓存最近"疑似广告"群消息,GKY 删完主人立刻 `/learnlast` 即可学到那条 + 踢发送者。
...
| `MSG_CACHE_SIZE` | `50` | 消息缓存环形缓冲保留最近多少条(1-500)。 |
...
1. **在群里回复那条广告消息,发 `/spam`**。机器人会:加黑+全群踢该用户、删消息,**并把广告内容学习入库**。
2. 学习了两样东西:
   - **整句指纹**:归一化后存库(去掉空格/标点/大小写/全半角差异)。以后任何人发**相同**广告(哪怕加了空格、标点、emoji 变体),都会被精确秒杀。
   - **自动提取的特征词**:从广告里挑出 ≥4 字中文 / ≥5 字英数的片段,加进词库 `general`。以后"改了几个字但保留特征词"的变体也能靠评分命中。
3. **只有主人 `/spam` 会学习**。普通群管理员 `/spam` 仍然加黑+踢,但不入库(避免管理员之间口径不一污染样本库)。
4. **管理样本库**:`/listsamples` 看学了哪些、`/delsample 序号` 删错学的、`/clearsamples confirm` 清空。
5. **防误杀**:归一化后 <6 字的太短样本不参与匹配;提取的特征词进 general 只 +2 分,单个词不达阈值(3)不会误杀正常用户。
6. 样本**无人为上限**(KV 单 value 25MB,可存几十万条),不用担心存满。
...
```
把广告文字(从截图打字、记忆、任何来源)直接粘给命令,立即学习指纹 + 关键词。

**② `/learnlast [N]`** — bot 自动缓存,一键学最近被删的:
- bot 会**自动缓存最近的"疑似广告"群消息**(含链接 / @提及 / 长数字 / 长文本的),正常闲聊不缓存
- GKY 删完广告后,主人立刻发 `/learnlast` → 学到那条 + **加黑并全群踢**发送者
- `/learnlast 3` 学最近 3 条
- 缓存空时提示"最近没有缓存到疑似广告"
- 学错了:`/delsample 关键词` 删样本 + `/unban TGID` 解封
...
- `MSG_CACHE_SIZE`(默认 `50`)— 环形缓冲保留最近多少条
