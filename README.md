...
> 全局黑名单语义：D1 中的黑名单是**永久全局黑名单**，**唯一写入来源是真人用 `/be`、`/sa` 指令**（外加可选的广告自动检测 `ad_auto`）；真人群内手动封禁、第三方机器人封禁一律不写入。命中后自助解封流程第一道闸即拒绝，机器人的任何流程都不会清除（包括 GKYbotSave 按钮代发、**群内管理员手动解封**——后者会被立即封回并通知主人）。只有**群管理员 / 超级管理员 / 主人 / 副主人**主动用 `/unban 用户ID` 指令才能从黑名单移除。
...
| `/check` | 群内回复 / 私聊 | 查询用户的 GKY 封禁记录,返回带按钮的二次审核。两种用法:**群内回复**某条消息发 `/check` 查该用户;**或 `/check TGID`**(私聊或群内均可)直接查指定 TGID,如 `/check 993005028`。结果附带**本地 D1 黑名单**明细:加黑方式 / **执行原因**(加黑时填写的原因,如 `/be 123 泄露订阅`) / 操作人 / 时间;加黑时未填原因则不显示该行 |
