...
- **`/ban` `/spam` `/unban` 支持批量**：分隔符兼容半角逗号 / 全角逗号 / 空格 / 换行，也支持 `[123,456,789]` 数组外壳。`/ban` `/spam` 1~19 个 TGID 即时执行，20 个及以上进入 D1 批量任务模式；`/unban` 保持单次最多 50 个。单条用法完全保持原样。
...
- **`/help` OWNER_IDS 专属帮助(隐藏指令展开)**:OWNER_IDS 中的主人/副主人**私聊**发 `/help` 可展开查看全部隐藏指令清单。所有 OWNER_IDS 专属命令(词库/样本/学习)对普通用户、群管理员、超级管理员**完全无反应**(群内静默、私聊提示权限不足且不泄漏指令内容)。这些命令本就不注册到 BotFather 命令菜单,天然隐藏。
...
> 全局黑名单语义：D1 中的黑名单是**永久全局黑名单**，命中后自助解封流程第一道闸即拒绝，机器人的任何流程都不会清除（包括 GKYbotSave 按钮代发）。只有管理员主动 `/unban 用户ID` 才能从黑名单移除。
...
| 命令 | 使用位置 | 说明 |
| --- | --- | --- |
| `/ban 用户ID [原因]` | 私聊 / 群内 | 加入 D1 黑名单（reason=manual）+ **遍历所有 GROUP_IDS 真踢出群**。支持单个、批量与原因：`/ban 123 广告引流`、`/ban 123,456 批量广告`、`/ban [123,456,789]`。1~19 个 TGID 即时执行；20 个及以上自动创建 D1 批量任务。群内授权管理员发送后，机器人会撤回命令消息本身，并发闪屏 + 私聊详情。 |
| `/unban 用户ID` | 私聊 / 群内 | 从 D1 黑名单移除。**支持批量**：`/unban 123,456,789` 或 `/unban [123,456,789]`（半角 / 全角逗号 / 空格 / 换行均可，单次最多 50 个）。群内执行时同样发闪屏 + 私聊详情。 |
| `/spam [用户ID] [原因]` | 群内回复 / 私聊 / 群内直接输入 | 回复消息时把被回复用户加入 D1 黑名单（reason=spam）+ **遍历所有 GROUP_IDS 真踢出群** + **删除被回复的垃圾消息**，回复后面的文本作为原因，例如回复广告发 `/spam 广告引流`。也支持直接输入 TGID：`/spam 123 广告引流`、`/spam 123,456 批量广告`、`/spam [123,456,789]`；20 个及以上自动创建 D1 批量任务。 |
| `/job 任务ID` | 私聊 / 群内 | 查询 20 个及以上 `/ban` `/spam` 大批量任务状态。群内查询会发闪屏并按通知规则私聊详情。 |
| `/jobrun 任务ID` | 私聊 / 群内 | 继续执行未完成的大批量任务。任务完成后会通知创建者/主人/副主人。 |
...
> 说明：`[123,456,789]` 数组外壳只用于粘贴批量 TGID，里面不解析原因；需要写原因时使用普通格式，例如 `/ban 123,456 批量广告`。`/job` 与 `/jobrun` 可直接发送使用，但不注册到 BotFather 命令菜单。
...
├── test_kick.mjs    # 真踢人闭环 + 广告学习 + 大批量任务离线测试（319 项）
├── README.md
└── LICENSE
```
...
| `batch_jobs` | 20 个及以上 TGID 的 `/ban` `/spam` 大批量任务状态、进度、统计和失败记录 |
...
**原因**：本项目只读取 D1。D1 表结构会由代码自动创建，但已有数据必须已经写入对应 D1 表。新部署后如果还没有创建过 20 个及以上 TGID 的 `/ban` `/spam` 大批量任务，`batch_jobs` 没有任务数据属于正常情况。
...
5. `/job 任务ID` / `/jobrun 任务ID` 用于查询或续跑 20 个及以上 `/ban` `/spam` 创建的大批量任务。
...
**原因**：当前有两套批量路径：`/ban` `/spam` 输入 1~19 个 TGID 时走即时流程；输入 20 个及以上 TGID 时自动创建 D1 大批量任务。`/unban` 不走大批量任务，仍受 `_worker.js` 的 `BATCH_LIMIT = 50` 限制。
**解决**：

1. `/ban` `/spam` 大名单可以直接粘贴，例如 `/ban [100,101,102,...]` 或 `/spam 100,101,102,... 批量广告`。20 个及以上会保存到 `batch_jobs`，按单轮 20 用户、最多 10 并发、失败重试 2 次执行。
2. 任务创建后会返回任务 ID；用 `/job 任务ID` 查询进度，用 `/jobrun 任务ID` 续跑未完成任务。
...
**是的,这是有意设计**。所有 OWNER_IDS 专属命令(`/help` `/importdefault` `/addword` `/delword` `/listwords` `/listsamples` `/delsample` `/clearsamples` `/learn` `/recent` `/learnlast`)的鉴权规则:

1. **只有 `OWNER_IDS` 中的主人/副主人能用**。普通用户、群管理员、超级管理员触发都不执行。
