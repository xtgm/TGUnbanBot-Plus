...
5. `/job 任务ID` / `/jobrun 任务ID` 用于查询或续跑 `/ban` `/spam` 自动创建的 D1 批量任务。
...
1. `/ban` `/spam` 大名单可以直接粘贴，例如 `/ban [100,101,102,...]` 或 `/spam 100,101,102,... 批量广告`。进入 D1 批量任务后会保存到 `batch_jobs`，按每轮最多 24 个“用户 × 群组”操作预算自动切片，最多 3 并发，避免 Cloudflare Worker 单次子请求超限。
