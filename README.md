...
- **`chat_member` 事件订阅**：管理员在群里手动**封禁**时自动加入 D1 黑名单；手动**解封**时，若对象仍在 D1 黑名单则**立即封回并通知主人**（群内手动解封无法绕过全局黑名单），仅非黑名单对象的解封做审计通知。
...
> 全局黑名单语义：D1 中的黑名单是**永久全局黑名单**，命中后自助解封流程第一道闸即拒绝，机器人的任何流程都不会清除（包括 GKYbotSave 按钮代发、**群内管理员手动解封**——后者会被立即封回并通知主人）。只有**群管理员 / 超级管理员 / 主人 / 副主人**主动用 `/unban 用户ID` 指令才能从黑名单移除。
...
- **管理员手动操作同步**：群管理员在群里手动 ban 时，机器人通过 `chat_member` 事件自动**加黑**；手动 unban 时，若对象仍在 D1 黑名单则**立即封回 + 通知主人**（全局黑名单不被群内手动解封清除），非黑名单对象仅做审计通知。移出全局黑名单请用 `/unban` 指令。
...
3. 检查 Cloudflare Worker 日志：每次有 `chat_member` 事件时会打印 `[chat_member] 同步加黑` 或 `[chat_member] 拦截群内手动解封(D1黑名单保护)` 日志。
