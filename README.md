...
- **广告词库存 KV + 主人热更新**:为隐私起见,代码内置词库**默认清空**(GitHub 公开仓库零敏感词),词库存在 KV 里。主人用 Telegram 命令热更新,**改词不用重新部署**:`/importdefault` 一键导入推荐词库、`/addword` 加词、`/delword` 删词、`/listwords` 查看。仅主人(OWNER_ID)可改(词库是全局配置,误加影响所有群)。
