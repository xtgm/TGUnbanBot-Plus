...
| [存储绑定](#存储绑定) | D1 数据库与 Queue 的绑定名和用途 |
| [部署](#部署) | **从零到跑起来的完整步骤**：建库、建队列、填变量、部署、初始化 Webhook |
...
├── test_kick.mjs    # 真踢人闭环 + Queue 完整回归（763 项）
...
| 绑定 | binding 名 | 用途 |
| --- | --- | --- |
| D1 Database | `DB` | **唯一存储后端**。黑名单、清扫缓存、批量任务全部存 D1。表结构由代码自动建立，首次写入自动建表。 |
...
| `SELF_UNBAN_APPROVED` | 解封成功文案（**能拿到群链接时用**） | 见 5.5 完整示例 | 用内置默认文案 |
| `SELF_UNBAN_APPROVED_NOLINK` | 解封成功文案（**拿不到群链接时的降级版**） | 见 5.5 完整示例 | 用内置默认文案 |
...
| 项目 | 要不要动 |
| --- | --- |
| D1 表结构 | ❌ **不用**。代码自动迁移（只按需补建缺失的表，不碰现有数据） |
