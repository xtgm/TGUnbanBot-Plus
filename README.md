...
- **D1 数据库（唯一存储后端）**：完全基于 Cloudflare D1 数据库，免费层每天 10 万次写入不会溢出。黑名单、广告词库、学习样本、消息缓存全部存 D1。
...
- **全局黑名单真踢人闭环**：`/ban` `/spam` 加黑后会**立即遍历所有 GROUP_IDS 把人踢出群**（之前只写库不踢人）；`/spam` 还会删除被回复的垃圾消息；黑名单用户在群里发言会被实时拦截删消息+踢人；被人拉回群也会立即被踢回去；新增 `/{TOKEN}/purge` / `/{TOKEN}/purge/run` 分批清扫存量数据。
...
| 变量名 | 说明 |
| --- | --- |
| `TOKEN` | 初始化入口密钥。访问 `https://你的Worker域名/{TOKEN}` 注册 Webhook + 注册命令；访问 `https://你的Worker域名/{TOKEN}/export` 导出黑名单；访问 `https://你的Worker域名/{TOKEN}/purge/run` 分批清扫存量黑名单仍在群里的人。建议用随机长字符串。 |
...
**用途**：当你从原项目升级（原项目只写黑名单不踢人）或修复了 bot 权限后，打开 `/purge/run` 把所有早期"加黑了但还在群里"的用户清扫掉。幂等可重复触发。
...
### Q5：D1 绑定后黑名单、词库或学习样本为空

**原因**：本项目只读取 D1。D1 表结构会由代码自动创建，但已有数据必须已经写入对应 D1 表。
**解决**：确认 `wrangler.toml` 或 Cloudflare Dashboard 已绑定 `DB`，并检查 D1 中的 `blacklist`、`ad_keywords`、`ad_samples`、`recent_messages`、`learn_snapshot` 表数据是否存在。绑定正确后无需额外接口，重新部署或重新访问 bot 即可使用 D1 数据。
...
3. `/spam` 可回复消息发送，也可直接 `/spam 用户ID`；回复消息时会额外删除被回复的垃圾消息。
