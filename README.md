...
3. `/sa`（旧称 `/spam`）命令额外删除被回复那条具体的垃圾消息，不受 48 小时限制（因为是按 `message_id` 直接删）。
4. **`/sa` 三种用法(引用回复 / `/sa 123` / `/sa 123 原因`)现已统一开启 `revoke_messages: true`**：加黑封禁的同时，Telegram 会自动撤回该用户在**所有配置群**近 48 小时内发的全部消息，无需再逐条手动删（旧版引用回复模式曾误设 `revoke_messages: false`，只删被回复那一条，靠缓存补删又存不全历史消息，已修复）。缓存清扫作为兜底，补删 revoke 偶尔漏掉的当前群近期消息。加黑命令 `/be`（旧称 `/ban`）走同一套封禁逻辑，行为一致。
