...
| `AD_CHECK_BIO` | `false` | 是否检测发言人简介 bio 里的引流广告(网址/t.me/卡网等)。开启后对群里发言人调 `getChat` 拉 bio 检测,命中即删+黑+踢。**有限制**:Telegram Bot API 只能拿到公开 username 或已与 bot 交互过用户的 bio,纯陌生用户拿不到(属尽力而为),且每个发言人多一次 API 调用有成本。默认关闭。名字/用户名检测不受此开关影响,始终生效。 |
