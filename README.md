...
- **发言人身份检测(本版重点,抓"名字/简介本身是广告"的号)**:有些广告号消息正文是正常内容(甚至蹭"chatgpt plus"这类正经词),但**名字或简介本身就是广告**(如 `сЛuВы Со ВпucoК`、简介写 `出租淫妻性奴`、`卡网:pay.ldxp.cn`)。本版**只检测发言人的身份字段(名字/用户名/简介),且只匹配明确的广告词,不碰消息正文**:① **名字/用户名**含身份广告词(卡网/卖号/车队/号商/出租淫妻/性奴/菠菜/博彩等露骨词)→ 直接秒杀,零 API 成本每条都查;② **简介 bio**(`AD_CHECK_BIO` **默认开启**)含同类广告词 → 调 `getChat` 拉 bio 检测,**带 KV 缓存**(同一用户默认 6 小时内不重复查,大幅降低 API 成本)。身份广告词可 `/addword identity <词>` 热更新,或 `/importdefault` 一键导入。
  - **⚠️ 绝不把"链接/@提及/域名"当广告(防误杀的核心)**:正常用户简介里放**双向机器人 `@xxxBot`**(Telegram 标准隐私用法)、**github 链接**、**个人频道 `t.me/xxx`**、**主页域名**都极其普遍。本版**只看露骨广告词**判定,光有链接/@/域名**一律放行**——技术讨论群、双向 bot 用户、分享 github 的人完全安全。(早期版本曾把"简介含链接"当广告导致大面积误杀正常技术用户,已彻底移除该逻辑。)
  - **参考杀神(GodKnowYouBot)实现 bio 检测**:杀神官网列出它筛查 first/last name、bio、头像、消息文本、图片、贴纸。本项目已对齐其中的**名字 + bio + 文本**检测(头像需 OCR,Worker 做不了)。诚实说明:bio 检测**不存在"完美"**——Telegram Bot API 的 `getChat` 只能拿到公开 username 或与 bot 交互过用户的 bio,**纯陌生群成员的 bio 杀神也拿不到**(它官网自承 1-3% 漏判率),这是平台限制,非代码可绕过。
  - **重要区分**:`chatgpt/plus/codex` 等是正经 AI 名字,**绝不会**被加进广告词;身份检测抓的是"简介写卡网/出租淫妻"这种正常用户绝不会有的露骨词。
...
| `AD_CHECK_BIO` | `true` | 是否检测发言人简介 bio 里的引流广告(网址/t.me/卡网等)。**默认开启**。对群里发言人调 `getChat` 拉 bio 检测,命中即删+黑+踢。**带 KV 缓存**(同一用户 6 小时内不重复查,大幅降低 API 成本)。**限制**:Telegram 只返回公开 username 或与 bot 交互过用户的 bio,纯陌生用户拿不到(尽力而为)。若担心活跃大群 API 量,可设 `false` 关闭。名字/用户名检测不受此开关影响,始终生效。 |
| 身份引流词 | 内置 | 只检测**发言人名字/简介**(不碰消息正文)的引流词:卡网/发卡/代充/车队/号商等。内置在 `_worker.js` 顶部 `DEFAULT_IDENTITY_SPAM_WORDS`;主人可 `/addword identity <词>` 热更新。正常聊"chatgpt plus"等正文不受影响。 |
