...
- **杀神全局库主动查杀 `GKY_ACTIVE_CHECK`（最接近杀神的做法，本版新增）**：反广告成熟 bot(如杀神)之所以"秒杀",核心不是读懂单条消息,而是**跨群账号信誉库**——某号在别的群发广告被标记,进任何群都被拦。本版开启后,**新成员进群时主动查一次 GKY 全局封禁库**,命中 → 删消息 + 全群封禁/预封 + 通知主人。等于白嫖杀神的跨群信誉数据,精度极高、不用猜内容。只在新成员进群触发(频率低,不撞子请求限制),管理员豁免,查询失败静默放行。独立开关,不依赖 `AD_FILTER_ENABLED`。
- **杀神库与本地 D1 黑名单分离记账 `GKY_SYNC_BLACKLIST`（默认 `false`）**:杀神全局库是**第三方的跨群信誉数据**,不是本项目的封禁决定,两套库默认**不合并**。命中仍照常删消息 + 全群封禁/预封(群照样受保护),但**不写入 D1 黑名单** —— D1 的唯一来源仍是真人 `/ban`、`/spam`、`/ad` 投票。三个必须分开的理由:① 第三方误标会直接变成你这边的**永久**黑名单,且用户**无法自助解封**;② D1 一旦有记录,该用户复入群时又触发一次"复入群拦截"通知,与杀神处置通知**重复轰炸主人**;③ 杀神那边撤销标记后你的 D1 记录不会跟着撤销,污染无法自动回收。确认某号确实是广告号需要永久加黑时,主人手动 `/ban <TGID>` 即可(通知里已给出可直接复制的指令)。仍想保留旧的自动并入行为,显式设 `GKY_SYNC_BLACKLIST=true`。
...
> 全局黑名单语义：D1 中的黑名单是**永久全局黑名单**，**唯一写入来源是真人用 `/ban`、`/spam` 指令**（外加可选的广告自动检测 `ad_auto`）；真人群内手动封禁、第三方机器人封禁一律不写入。命中后自助解封流程第一道闸即拒绝，复制或发送 `GKYbotSave` 只处理 GKY，不会清除本地 D1；群内管理员手动解封也会被立即封回并通知主人。只有**群管理员 / 超级管理员 / 主人 / 副主人**主动用 `/unban 用户ID` 指令才能从黑名单移除。
...
| `/addgroup -100xxxx [备注]` | **仅第一主人私聊** | **不改环境变量、直接用指令新增治理群组**。写入 D1，与 `GROUP_ID` 环境变量分离存放（Worker 无法写自己的环境变量），但合并后享受**完全相同**的治理能力：黑名单拦截、复入群踢回、广告自动检测、`/ban` `/spam` 全群封禁、`/purge` 清扫、批量任务。<br>三道硬校验：① ID 必须 `-100` 开头；② 不得与 `GROUP_ID` 环境变量重复；③ **bot 必须已是该群管理员且有「封禁用户」权限**（复用 `/purge` 预检逻辑），否则拒绝并说明原因 —— 加进来也只会全程报错。 |
...
| `GKY_ACTIVE_CHECK` | `false` | **杀神全局库主动查杀(最接近杀神的做法)**。设为 `true` 后在**两个时机**查 GKY 全局封禁库(`GKY_BANLIST_ENDPOINT`),命中 → 删消息 + 全群封禁/预封 + 通知主人:<br>① **新成员进群时**——某号在别的群发广告被杀神标记,一进你群就被拦;<br>② **发言时**（本版新增）——存量老号早就在群里、不会触发①,但它在别的群有前科。实测漏杀的号大多属于这种（例如只发一条 `@xxx_bot`，消息本身零广告特征，GKY/nmBot 都杀了而本项目漏）。<br>**为什么它不会误封**：判据是「这个号有没有前科」这个**外部信誉数据**，完全不做文字匹配，所以不存在正则误判；管理员豁免；查询失败静默放行。<br>**成本控制**：发言时只对「疑似消息」查（含链接/@提及/长数字/名片），且结果带缓存（命中 30 分钟 / 干净号 6 小时 / 失败 5 分钟），正常闲聊零外部请求。<br>独立开关,不依赖 `AD_FILTER_ENABLED`。**是否同时写入本地 D1 黑名单由 `GKY_SYNC_BLACKLIST` 控制,默认不写。** |
| `GKY_SYNC_BLACKLIST` | `false` | **杀神命中是否并入本地 D1 黑名单**。默认 `false` = 两套库分离记账:杀神命中照常全群封禁/预封,但**不写 D1**,D1 的唯一来源仍是真人 `/ban`、`/spam`、`/ad` 投票。这样第三方误标不会变成你这边的永久黑名单(用户也就不会因此无法自助解封),复入群时也不会再触发与杀神通知重复的"复入群拦截"轰炸。需要永久加黑时主人手动 `/ban <TGID>`(通知里已附可复制指令)。设为 `true` 恢复旧的自动并入行为(reason 记为 `gky_global`)。 |
| `CROSS_GROUP_SPAM_CHECK` | `false` | **跨群刷屏检测（不看文案，只看行为）**。设为 `true` 后：同一账号在 **≥2 个配置群**、**30 分钟内**发出**高度相似**内容 → 判定刷屏广告，删消息 + 写入 D1 黑名单（reason `cross_group_spam`）+ 全群封禁并撤回历史发言 + 通知主人。<br>**为什么它抗改词**：判据是「同一个号在你多个群贴几乎一样的话」这个**行为**，不是文案里有什么词。广告号改措辞、换繁体、插空格、换金额、换 @账号全都躲不掉（相似度引擎会洗掉链接/数字/@账号/金额只比对骨架）；而正常人不会在你好几个群刷同一段话。<br>**免费版额度安全**：只对「疑似消息」记指纹（含链接/@提及/长数字/名片，复用 `looksLikeAdCandidate`），正常闲聊完全不写库。实测每天 1 万条群消息时 D1 写入仅占免费额度约 10%。<br>**七重防误判**：疑似消息门槛、骨架 ≥12 字、跨 ≥2 群、Dice ≥0.9（比学习样本 0.82 更严）、30 分钟窗、管理员豁免、代理内容豁免。检测异常一律放行，绝不阻断消息处理。 |
| `CROSS_GROUP_SPAM_MIN_GROUPS` | `2` | 跨几个群才算刷屏。最小值 2；填 `3` 更保守（要在 3 个群都刷到才处置，漏杀增加但误判更低）。 |
...
> 未绑定 D1 或 D1 黑名单读取失败时，自助解封会失败关闭并拒绝继续，避免绕过全局黑名单。`/ban` `/spam` `/unban` `/blacklist` 等命令会提示未绑定存储空间。未绑定 `BULK_QUEUE` 时，大批量任务仍会创建到 D1，但不会自动续接，需要管理员用 `/jobrun 任务ID` 手动补跑。
...
| `cross_group_posts` | 跨群刷屏检测的疑似消息指纹（仅 `CROSS_GROUP_SPAM_CHECK=true` 时写入，正常闲聊不入库，超窗自动裁剪） |
...
| `GKY_ACTIVE_CHECK` | `true` | **强烈建议开**。新成员进群时 + 发言时各查一次杀神全局封禁库，白嫖跨群信誉数据。这是拦「消息本身没有广告特征、但号有前科」的唯一手段，且完全不做文字匹配、不会误判 |
| `CROSS_GROUP_SPAM_CHECK` | `true` | **强烈建议开**（多群部署时尤其）。同一号在 ≥2 个配置群刷相似内容即判广告。纯行为判据，广告号改词/换繁体/换金额都躲不掉；只记疑似消息，免费版额度也安全 |
| `GKY_SYNC_BLACKLIST` | **不填**（默认 `false`） | 保持默认。杀神命中不写你的 D1 黑名单，避免第三方误标变成你这边的永久封禁 |

> **关于「漏杀」的现实说明**：`AD_FILTER_ENABLED` 那套（词库/指纹/相似度）本质是文字匹配，对**同义改写**天然有边界 —— 广告商把「洗钱」换成别的说法就可能漏。真正稳的是**不看文案的两条**：`GKY_ACTIVE_CHECK`（查账号前科）和 `CROSS_GROUP_SPAM_CHECK`（查跨群刷屏行为）。如果你被漏杀困扰，优先把这两个开起来，它们不会因为对方改措辞而失效。
...
| 项目 | 要不要动 |
| --- | --- |
| D1 表结构 | ❌ **不用**。代码自动迁移（schema v5→v6，只新建 `cross_group_posts` 表，不碰现有数据；更早版本升级会一并补建 `dynamic_groups`） |
