...
section('[12] 修复项专项：manual 提权 / 自身 username / 回复学习词表');
{
	...
	// 否则「苹果、香蕉、橘子、西瓜」这类正常句子会被整片误伤（离线实测确认过）。
	for (const sample of [
		'pq低.價.出.正.品.水.果.機 pp',
		'ci7.折.出.水.果.機.轉.手.掙.3K bz',
		'y8轉.手.一.臺.搞3.K 5v',
		'3v線.下.可.驗.正.品.保.真 dp',
		// 多字分组变体：只认「严格单字」时整条漏检，广告号并不严格逐字切。
		'sh最新.17水.果僅.需五.千.多 kn'
	]) {
		assert('混淆检测：真实广告样本命中 [' + sample.slice(0, 12) + ']',
			W.hasAdCharSplitObfuscation(sample) === true);
	}
	// 顿号逐字切仍要抓到（每组恰好一个汉字，正常人不这样写）。
	assert('混淆检测：顿号逐字切命中', W.hasAdCharSplitObfuscation('联、系、我、们、私聊') === true);
	assert('混淆检测：顿号逐字切可还原',
		W.deobfuscateAdCharSplit('联、系、我、们、私聊') === '联系我们私聊');
	// 正常文本零误伤 —— 每一条都是放宽分组后真实踩到过的坑。
	for (const safe of [
		'今日 入门 教程分享',
		'这个方案不错，我觉得可以试试',
		'U.S.A 的缩写是这样写的',
		'版本号是 1.2.3.4 对吧',
		'IP 是 192.168.1.1',
		'文件名 test.tar.gz 解压看看',
		// 以下四条是中文列举，天然「2 字 + 顿号」重复三次以上，必须放行。
		'苹果、香蕉、橘子、西瓜都买了',
		'张三、李四、王五、赵六来了',
		'上海、北京、广州、深圳',
		'第一步、第二步、第三步、第四步'
	]) {
		assert('混淆检测：正常文本放行 [' + safe.slice(0, 12) + ']',
			W.hasAdCharSplitObfuscation(safe) === false);
	}
	// A2：去混淆文本参与词表匹配。挂在 countAdKeywordHits 这个唯一入口上，
	// 一处改动让结构评分 / 指纹抽取 / 豁免词 / 引用体查杀全部覆盖混淆变体。
	assert('混淆 A2：去混淆后简体词表命中',
		W.countAdKeywordHits('低.价.出.正.品.手.机', ['低价']).length === 1);
	assert('混淆 A2：原文不成签名时不还原、不凭空命中',
		W.countAdKeywordHits('今日 入门', ['日入']).length === 0);
	// A1：结构加分。这批繁体样本词表一个都不命中（项目刻意不收繁体），
	// 4 分全部来自混淆签名本身 —— 这正是「打规避行为、不打广告词汇」的价值。
	assert('混淆 A1：正文混淆加 4 分',
		W.scoreAdMessageText('pq低.價.出.正.品.水.果.機 pp', {}).score === 4);
